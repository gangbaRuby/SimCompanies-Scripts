...
(function () {
    ...
    const SCobgUIManager = {
        ...
        renderContent(l1, l2, l3, items) {
            ...
            const isSeasonalSet = l1 === '背景和环境' && l2 === '季节性';
            const isLevelSet = l1 === '建筑外观' && (l2 === '生产建筑' || l2 === '零售建筑');
            const showSetOptions = isSeasonalSet || isLevelSet;
            const setOptions = showSetOptions ? this.getSetOptions(items) : [];
            const setbarHtml = showSetOptions ? `
                <div class="scobg-setbar">
                    <span class="scobg-setbar-label">${isSeasonalSet ? '整套替换' : '全等级应用'}</span>
                    ${setOptions.map(name => `<button class="scobg-set-btn" data-set="${name}">${name}</button>`).join('')}
                    <button class="scobg-set-btn" data-set="__reset__">${isSeasonalSet ? '整套还原' : '全等级还原'}</button>
