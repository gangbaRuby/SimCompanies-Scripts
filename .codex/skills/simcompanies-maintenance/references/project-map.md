...
- `chatMessageBlocker.js`：聊天室全局屏蔽（WebSocket 实时消息按 sender.id 过滤 + 消息组「屏蔽」按钮 + CSS 零高隐藏 + 导入当前领域游戏内黑名单；不改 HTTP 响应以防 fullHistory `r.length<30` 误判；名单全局键 `SC_ChatBlock_List`）。
