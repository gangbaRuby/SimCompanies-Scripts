// ==UserScript==
// @name         自动计算最大时利润 (iOS兼容版)
// @namespace    https://github.com/gangbaRuby
// @version      1.32.20-iOS
// @license      AGPL-3.0
// @description  在商店计算自动计算最大时利润，在合同、交易所展示最大时利润（iOS懒加载优化版）
// @author       Rabbit House
...
(function () {
    ...
    // ======================
    // iOS/WKWebView 环境检测（iOS WebView 无 JIT、内存受限、localStorage 7天过期）
    // ======================
    const isIOSWebView = (() => {
        const ua = navigator.userAgent || '';
        // 检测 iOS 设备 + 非 Safari（Tampermonkey iOS 使用 WKWebView，UA 可能包含 Safari）
        const iosDevice = /iPhone|iPad|iPod/.test(ua);
        // 进一步：iOS 上 Tampermonkey 的 WebView 通常不支持 ServiceWorker/SharedWorker
        const noSharedWorker = typeof SharedWorker === 'undefined';
        // 检测是否为受限 WebView（memory < 设备标称值的显著比例）
        const lowMemory = (() => {
            // navigator.deviceMemory 在 iOS 通常为 undefined
            if ('deviceMemory' in navigator && navigator.deviceMemory < 4) return true;
            return iosDevice; // iOS 一律视为低内存
        })();
        return iosDevice || (noSharedWorker && lowMemory);
    })();
    const LOG_PREFIX = isIOSWebView ? '[iOS兼容模式]' : '';

    // 通用懒 Worker 工厂：只在首次 postMessage 或访问 onmessage 后才真正创建 Worker
    // 用法：const profitWorker = lazyWorker(workerCodeString);
    //        profitWorker.onmessage = fn;  // 可在创建前设置
    //        profitWorker.postMessage(msg); // 触发真正创建
    const lazyWorker = (workerCode) => {
        let _worker = null;
        let _onMsg = null;
        let _onErr = null;
        ...
        const ensureWorker = () => {
            ...
            try {
                _worker = new Worker(URL.createObjectURL(new Blob([workerCode], { type: 'application/javascript' })));
                if (_onMsg) _worker.onmessage = _onMsg;
                if (_onErr) _worker.onerror = _onErr;
            } catch (e) {
                console.warn(LOG_PREFIX, 'Worker 创建失败，降级为空操作', e);
                _worker = { postMessage: () => {}, onmessage: null, onerror: null, terminate: () => {} };
                if (_onMsg) _worker.onmessage = _onMsg;
            }
            return _worker;
        };
        ...
        return {
            postMessage(msg) { ensureWorker().postMessage(msg); },
            terminate() { if (_worker) { _worker.terminate(); _worker = null; } },
            get onmessage() { return _worker ? _worker.onmessage : _onMsg; },
            set onmessage(fn) { _onMsg = fn; if (_worker) _worker.onmessage = fn; },
            get onerror() { return _worker ? _worker.onerror : _onErr; },
            set onerror(fn) { _onErr = fn; if (_worker) _worker.onerror = fn; },
        };
    };
    ...
    `;
        ...
        const profitWorker = lazyWorker(workerCode);
    ...
    const incomingContractsHandler = (function () {
        ...
        `;
        const marketProfitWorker = lazyWorker(marketWorkerCode);
    ...
    (function () {
        ...
        function createProfitWorker() {
            ...
            `;
            const worker = lazyWorker(workerCode);
            return worker;
        }
