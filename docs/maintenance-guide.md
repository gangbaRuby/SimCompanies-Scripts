...
默认版本规则为 `1.x.y` 到 `1.x.(y+1)`。命令会同步源码和包元数据版本、更新 `CHANGELOG.md`、生成正式 `autoMaxPPHPL.user.js`、移除名称中的 `(DEV)`，并在产物末尾写入 `// @changelog`。它不会自动提交、推送、上传或部署。
...
分发前确认正式产物包含预期功能、正确版本、没有 `(DEV)`、包含本次更新说明，并保留正确的更新和下载地址。Git 提交与更新服务器部署属于单独、需明确授权的步骤。
